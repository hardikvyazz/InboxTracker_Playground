...
function loadLastProcessedTimestamp(): number | null {
    if (fs.existsSync(LAST_PROCESSED_FILE)) {
        ...
        return (JSON.parse(data) as TokenData).lastProcessed || null;
    }
    return null;
}

function saveLastProcessedTimestamp(timestamp: number): void {
    const data: TokenData = { lastProcessed: timestamp };
...
async function authorize() {
    ...
    return new Promise<google.auth.OAuth2>((resolve, reject) => {
...
async function processReports(auth: google.auth.OAuth2) {
